...
export default {
   ...
   */
  getDvdAppEnv() {
    let result = /(ios|android)\.(davdian\.com|vyohui\.cn|bravetime\.net)\/([\d\.]+)\/(dev|beta|gray)/.exec(navigator.userAgent);
    if (result) {
      return result[4];
  ...
  getDvdAppNum() {
    let result = /(ios|android)\.(davdian\.com|vyohui\.cn|bravetime\.net)\/([\d\.]+)\/(dev|beta|gray)\/dvddomain=(\d+)/.exec(navigator.userAgent);
    if (result) {
      return result[5] || '';
    }
  },
   ...
   */
  getSysVersion(){
    let result = /(?:iPhone OS ([\d_]+))|(?:Android ([\d\.]+))/ig.exec(navigator.userAgent);
   ...
   */
  getDvdAppDeviceId(){
    let result = /\/deviceId=([^/ =]+)/.exec(navigator.userAgent);
