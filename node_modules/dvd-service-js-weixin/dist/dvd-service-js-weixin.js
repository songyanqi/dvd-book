...
var _dvdServiceJsTj = require('dvd-service-js-tj');
...
var config = {
    ...
    defaultShareInfo: {
        title: 'MAMA+|大V店', // 分享标题
        desc: 'MAMA+|大V店', // 分享描述
        link: _dvdBaseJsRuntime2.default.isClient() && window.location.href, // 分享链接
...
exports.default = {
     ...
     * 分享接口
     * */
    getwxjssdk: function getwxjssdk(callback) {
        //todo 判断是否已经加载jssdk
        if (wxjssdk) {
            callback('success');
        } else {
    ...
    setShareInfo: function setShareInfo(param, response) {
        ...
        } else if (_dvdBaseJsRuntime2.default.isClient() && window && window.shareSource) {
        ...
        function shareInfoPost(msg) {
            ...
            if (shareSource) {
                ...
                if (_dvdBaseJsRuntime2.default.isClient() && window.gio) {
     ...
     * @param {Bool} tag true为直接跳转，false为请求php
     * @param {Object} parameter 任意参数到结果页逻辑自行处理
     */
    goAuthPage: function goAuthPage(show, tag, parameter, isAutoLogin) {
        if (_dvdServiceJsLogin2.default.isLogined()) {
            return;
        }

        var url = '';
        if (isAutoLogin) {
            url = '/wxlogin/wechatlogin.html?referer=' + encodeURIComponent(location.href) + '&isAutoLogin=1';
        } else {
            url = '/wxlogin/wechatlogin.html?referer=' + encodeURIComponent(location.href);
        }
